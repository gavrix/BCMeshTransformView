...
- (BOOL)loadProgram
{
    ...
    vertShaderPathname = [[NSBundle bundleForClass:[self class]] pathForResource:[self shaderName] ofType:@"vsh"];
    if (![self compileShader:&vertShader type:GL_VERTEX_SHADER file:vertShaderPathname]) {
        NSLog(@"Failed to compile vertex shader \"%@\"", [self shaderName]);
        return NO;
    }
    
    fragShaderPathname = [[NSBundle bundleForClass:[self class]] pathForResource:[self shaderName] ofType:@"fsh"];
